...
For Codex, Antigravity, Copilot, and OpenCode the hook surface exists, but no documented channel passes the parent's own session id to the `agent-loop role` init call, so a session-keyed guard cannot be wired yet on a documented basis. One undocumented channel exists: Codex injects `CODEX_THREAD_ID` into its shell tool environment (verified in openai/codex source, `codex-rs/core/src/exec_env.rs`), and the PreToolUse payload's `session_id` is that same thread id, so a Codex guard is wireable today against that variable. Deferred until the variable is documented; add a guard only when a session-id channel is documented for that harness.
