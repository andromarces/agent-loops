...
4. Stage releases from a GitHub Actions workflow on a published GitHub release or a manual dispatch, with `--provenance` and the `id-token: write` permission, on Node 24 so the bundled npm CLI supports staged publishing and trusted publishing. A release event and a manual dispatch must run on the tag `v<version>`, so the recorded provenance names the tag. The workflow authenticates with OIDC trusted publishing, stores no npm token, and runs `npm stage publish`; a maintainer approves each staged version with 2FA. npm requires the package to exist before a trusted publisher can be configured, so the first version is published once from a developer checkout.
