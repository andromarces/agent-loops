...
- The state file records `task`, `mode`, `cwd`, `parentSession`, `maxSteps`, `stepsUsed`, `lifecycle`, `roles.{worker,reviewer}.{kind,model,effort,sessionId}`, `lastDispatch`, and `lastResult`. Updates are atomic (temp file plus rename); exclusive access uses `state.lock` with a stale-lock check on the owner pid.
- Lifecycle values: `active`, `dispatched`, `interrupted`, `halted`, `finished`, `aborted` (terminal: `halted`, `finished`, `aborted`). A turn interrupted between the CLI start and the state write leaves `dispatched` with a dead lock owner; the next call marks it `interrupted` and refuses to repeat the turn. From `interrupted`, only `abort` or an explicit `dispatch --resume-interrupted` is accepted.
...
- The reviewer is required to end with one explicit `Verdict:` line (`accept` or `reject`). A missing or malformed line yields `verdict: unknown`; process success never implies acceptance.
