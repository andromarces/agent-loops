...
- The shared `~/.agents/skills` folder exposes the Codex skill to Copilot CLI and OpenCode, and OpenCode also discovers `~/.claude/skills`. Each shared skill must identify the running harness by process ancestry: nested harnesses inherit session variables, and a foreign harness leaves `${CLAUDE_SESSION_ID}` unexpanded. Both the Codex and Claude skills refuse to start a run when the nearest harness process is not theirs. Both shared skills set `metadata.opencode/autoinvoke: false` and `slash: false`, so OpenCode hides them from its model list and interactive command catalog, and the OpenCode plugin command is the only `/agent-loop` entry point that carries the OpenCode session id.
